...
Begge applikationer opretter en lokal SQLite-database med tabellerne `books` og `users`. Der seedes tre bøger og tre brugere ved første opstart. Søgefunktionen bruger kun `books`-tabellen og returnerer bogens titel og forfatter.
...
1. Kald `/search` med en almindelig søgetekst i begge applikationer.
